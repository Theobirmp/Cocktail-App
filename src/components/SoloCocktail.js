...
const SoloCocktail = ({
  ...
  function handleClick(e) {
    //add a cocktail into the favourites
    e.preventDefault();
    ...
    if (favourite === false) {
      ...
      tempFavouriteCocktails = tempFavouriteCocktails.filter(
        (coc) => coc.id !== id
      );
    }
  ...
  return (
    <div className="fav-drink solo-drink">
      ...
      <div className="ingredients-container">
        <b className="main-ingredients">Main Ingredients:</b>
        <ul className="main-ingredients-ul">
          {ingredients.map((ingre) =>
            ingre ? (
              <li className="instructions" key={uuidv4()}>
                {ingre}
              </li>
            ) : (
              ''
      ...
      {instructions.length >= 100 ? (
        <>
          <p ref={lessInstructions} className="instructions">
            {instructions.substring(0, 100)}...
            <button className="show-more-less-button" onClick={ShowMore}>
              Read More
            </button>
          </p>
          <p ref={moreInstructions} className="instructions display-none">
            {instructions}
            <button className="show-more-less-button" onClick={ShowMore}>
              Read Less
            </button>
